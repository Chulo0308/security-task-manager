...
export default function SitePage() {
  ...
  const [editFloorForm, setEditFloorForm] = useState({ name: "", level: "" });

  const startEditFloor = (f: Floor) => {
    setEditingFloorId(f.id);
    setEditFloorForm({ name: f.name, level: String(f.level) });
  };
  ...
  const saveFloor = async (id: string) => {
    if (!editFloorForm.name.trim()) return;
    const res = await fetch(`/api/site/floors/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: editFloorForm.name, level: Number(editFloorForm.level) || 0 }),
    });
    if (res.ok) {
      const data = await res.json();
      setFloors((cur) => cur.map((f) => (f.id === id ? data.floor : f)));
      setEditingFloorId(null);
    }
  };
    ...
    <div className="max-w-[1000px] mx-auto px-4 lg:px-8 py-6 lg:py-10">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="max-h-[420px] overflow-y-auto divide-y divide-slate-100">
          {floors.map((f) =>
            editingFloorId === f.id ? (
              <div key={f.id} className="px-6 py-2.5 flex items-center gap-2 bg-indigo-50/50">
                <input
                  value={editFloorForm.level}
                  onChange={(e) => setEditFloorForm({ ...editFloorForm, level: e.target.value })}
                  className="w-16 px-2 py-1 border border-slate-200 rounded text-xs"
                  placeholder="Level"
                />
                <input
                  value={editFloorForm.name}
                  onChange={(e) => setEditFloorForm({ ...editFloorForm, name: e.target.value })}
                  className="flex-1 px-2 py-1 border border-slate-200 rounded text-sm"
                  placeholder="Floor name"
                  autoFocus
                />
                <button
                  onClick={() => saveFloor(f.id)}
                ...
                {isAdmin && (
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-1.5 rounded-md text-slate-300 hover:text-indigo-600 hover:bg-indigo-50"
                    >
